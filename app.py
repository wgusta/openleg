...
def _execute_approved_action(decision):
    ...
    if activity == 'outreach':
        ...
        if inbox == 'lea' and AGENTMAIL_LEA_INBOX:
            msg_id = agentmail_client.send_message(inbox_id=AGENTMAIL_LEA_INBOX, to=to, subject=subject, text=text)
            sent = msg_id is not None
        else:
            sent = send_email(to, subject, text)
        if sent:
            db.track_event(
                'approved_email_sent', data={'to': to, 'subject': subject, 'request_id': decision.get('request_id')}
            )
    ...
    if activity == 'update_consent':
        ...
        fields = {
            ...
            if k in ('share_with_neighbors', 'share_with_utility', 'updates_opt_in') and v is not None
        }
        ...
        try:
            ...
            db.track_event(
                'lea_update_consent',
                data={'building_id': building_id, 'fields': fields, 'request_id': decision.get('request_id')},
            )
    ...
    if activity == 'generate_leg_document':
        ...
        try:
            ...
            req = urllib.request.Request(
                'http://localhost:5000/api/formation/generate-document',
                data=body,
                headers={'Content-Type': 'application/json'},
            )
            ...
            db.track_event(
                'lea_generate_leg_document',
                data={'community_id': community_id, 'doc_type': doc_type, 'request_id': decision.get('request_id')},
            )
...
def collect_building_locations(city_id=None, exclude_building_id=None):
    ...
    for b in buildings:
        ...
        locations.append({'lat': jlat, 'lon': jlon, 'type': b.get('user_type', 'anonymous')})
    return locations
...
def find_provisional_matches(new_profile, city_id=None):
    ...
    members = [
        {'building_id': p.get('building_id', ''), 'lat': float(p['lat']), 'lon': float(p['lon'])} for p in provisional
    ]
    return {
...
def vnb_transparenz():
    ...
    return render_city_template(
        'vnb_transparenz.html',
        rankings=rankings,
        kantons=kantons,
        kanton_filter=kanton_filter,
        year=year,
...
def sitemap_xml():
    ...
    pages = [
        ...
        ('/leg-gruenden', '0.8', 'weekly', current_date),
...
def admin_strategy():
    ...
    if 'text/html' in (request.headers.get('Accept') or ''):
        return render_template('admin/strategy.html', stats=stats, email_stats=email_stats, pipeline=pipeline)
...
def api_internal_send_email():
    ...
    if inbox == 'lea' and AGENTMAIL_LEA_INBOX:
        msg_id = agentmail_client.send_message(
            inbox_id=AGENTMAIL_LEA_INBOX, to=to, subject=subject, text=text, html=html_body
        )
        sent = msg_id is not None
    else:
        sent = send_email(to, subject, text, html=bool(html_body), from_email=None)
    # Log + notify
    db.track_event('email_sent', data={'to': to, 'subject': subject, 'inbox': inbox, 'sent': sent})
...
def webhook_telegram():
    ...
    if cmd == 'reset-lea':
        ...
        _send_telegram_message(
            '\u2705 LEA circuit breaker reset. All budgeted tools restored.', reply_to_message_id=msg.get('message_id')
        )
        return jsonify({'ok': True})
    ...
    if not decision:
        _send_telegram_message(f'No pending request `{request_id}` found.', reply_to_message_id=msg.get('message_id'))
        return jsonify({'ok': True})
...
def api_check_potential():
    ...
    if not cluster_info:
        return jsonify(
            {'potential': False, 'message': 'Keine direkten Partner gefunden.', 'profile_summary': estimates}
        )
    return jsonify(
        {'potential': True, 'message': 'Partner gefunden!', 'cluster_info': cluster_info, 'profile_summary': estimates}
    )
...
def api_register_anonymous():
    ...
    event_hooks.fire(
        'registration', {'building_id': building_id, 'city_id': city_id, 'email': email, 'type': 'anonymous'}
    )
...
def api_register_full():
    ...
    event_hooks.fire(
        'registration', {'building_id': building_id, 'city_id': city_id, 'email': email, 'type': 'registered'}
    )
...
def api_meter_data_upload():
    ...
    db.save_data_consent(
        building_id, tier=tier, share_municipality=True, share_research=(tier >= 2), share_providers=(tier >= 3)
    )
...
def api_public_stats():
    ...
    stats = db.get_stats(city_id=city_id)
    return jsonify(
        {'total_users': stats.get('total_buildings', 0), 'registrations_today': stats.get('registrations_today', 0)}
    )
...
def api_cron_process_billing():
    ...
    for community in communities:
        ...
        consumption_df = pd.DataFrame(
            [
                {
                    bid: consumption_frames[ts].get(bid, 0)
                    for bid in building_ids
                    if bid in consumption_frames.get(ts, {})
                }
...
def api_cron_seed_municipalities():
    ...
    result = municipality_seeder.seed_all_municipalities(kanton_filter=kanton, provision_tenants=provision_tenants)
    return jsonify(result)
