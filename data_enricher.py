...
def mock_get_gwr_data(lat, lon, plz):
    ...
    if plz in [5430, 5432]:
        return 'EFH', 1, 160  # Wettingen/Neuenhof
    elif plz == 5400:
        return 'MFH', 8, 700  # Baden Zentrum
    else:
        return 'EFH', 1, 150  # Default


def mock_get_plz_stats(plz):
    """Simuliert eine Statistik-Abfrage (EV-Dichte, Kaufkraft)"""
    if plz == 5400:
        return 15.0, 1.2  # Baden
    elif plz == 5430:
        return 10.0, 1.0  # Wettingen
    else:
        return 5.0, 0.9  # Umland
...
def estimate_consumption_kwh(building_type, num_apartments, energy_surface_m2, plz_stats):
    ...
    if building_type == 'EFH':
        ...
        consumption = num_apartments * 2500
    else:
        consumption = energy_surface_m2 * 20  # Gewerbe-Heuristik
...
def get_energy_profile_for_address(address_string):
    ...
    final_profiles = ml_models.generate_mock_profiles(
        annual_consumption_kwh=base_consumption_kwh_pa + ev_kwh_pa, potential_pv_kwp=pv_kwp
    )
...
def get_mock_energy_profile_for_address(address_string):
    ...
    final_profiles = ml_models.generate_mock_profiles(
        annual_consumption_kwh=annual_consumption_kwh, potential_pv_kwp=potential_pv_kwp
    )
