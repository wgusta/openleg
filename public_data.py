...
def fetch_elcom_tariffs(bfs_number: int, year: int = 2026) -> List[Dict]:
    ...
    try:
        resp = requests.post(
            LINDAS_ENDPOINT, data={'query': sparql}, headers={'Accept': 'application/sparql-results+json'}, timeout=30
        )
...
def fetch_elcom_municipalities(year: int = 2026) -> List[Dict]:
    ...
    try:
        resp = requests.post(
            LINDAS_ENDPOINT, data={'query': sparql}, headers={'Accept': 'application/sparql-results+json'}, timeout=60
        )
        resp.raise_for_status()
        data = resp.json()
        results = []
            ...
            results.append(
                {
                    ...
                    'solar_potential_pct': _safe_float(
                        row.get('anteil_dachflaechen_solar') or row.get('solar_potential_pct')
                    ),
                    'ev_share_pct': _safe_float(row.get('anteil_ev') or row.get('ev_share_pct')),
                    'renewable_heating_pct': _safe_float(
                        row.get('anteil_erneuerbar_heizen') or row.get('renewable_heating_pct')
                    ),
                    'electricity_consumption_mwh': _safe_float(
                        row.get('stromverbrauch_mwh') or row.get('electricity_consumption_mwh')
                    ),
                    'renewable_production_mwh': _safe_float(
                        row.get('erneuerbare_produktion_mwh') or row.get('renewable_production_mwh')
                    ),
                }
            )
            ...
            results.append(
                {
                    ...
                    'suitable_roof_area_m2': _safe_float(
                        row.get('dachflaeche_geeignet_m2') or row.get('suitable_roof_area_m2')
                    ),
