...
def register():
    ...
    muni_id = db.save_municipality(
        bfs_number=int(bfs), name=name, kanton='ZH', dso_name=dso, population=population, subdomain=subdomain
    )
...
def dashboard():
    ...
    return render_template('gemeinde/dashboard.html', municipality=muni, stats=stats, dash=dash_stats, error=None)
...
def formation():
    ...
    return render_template('gemeinde/formation.html', municipality=muni, community_id=community_id, error=None)
...
def profil(bfs):
    ...
    return render_template(
        'gemeinde/profil.html', profile=profile, tariffs=tariffs, solar=solar, value_gap=value_gap, h4_tariff=h4
    )
...
def verzeichnis():
    ...
    return render_template('gemeinde/verzeichnis.html', profiles=profiles, kanton=kanton, query=q, sort=order_by)
