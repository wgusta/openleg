...
def calculate_community_autarky_details(
    ...
    for _, row in community_buildings_df.iterrows():
        ...
        confidence_values.append(
            max(0.0, min(1.0, _to_float(profile_meta.get('quality_score'), MOCK_PROFILE_CONFIDENCE)))
        )
...
def find_optimal_communities(
    ...
    if building_data_df.empty or len(building_data_df) < min_community_size:
        print(
            f'[ML] Zu wenig Daten für Clustering (min. {min_community_size} benötigt, {len(building_data_df)} vorhanden).'
        )
