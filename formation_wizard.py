...
def get_contract_templates(jurisdiction='Kanton Zürich', dso_contact='EKZ Verteilnetz AG'):
    ...
    return {
        ...
        'participant_contract': {
            ...
            'jurisdiction': jurisdiction,
            'language': 'de',
            'sections': ['participant_info', 'community_info', 'obligations', 'payment_terms', 'termination'],
        },
        'dso_notification': {
            'title': 'Anmeldung Lokale Elektrizitätsgemeinschaft',
            'recipient': dso_contact,
            'form_id': 'LEG-DSO-001',
            'sections': ['community_details', 'participants', 'grid_connection', 'metering_setup', 'start_date'],
        },
    }
...
def create_community(
    db, name: str, admin_building_id: str, distribution_model: str = 'simple', description: str = ''
) -> Optional[Dict]:
    """
    ...
    try:
        with db.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Track event
                db.track_event('member_invited', building_id, {'community_id': community_id, 'invited_by': invited_by})
    ...
    try:
        with db.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if cur.rowcount > 0:
                    db.track_event('member_confirmed', building_id, {'community_id': community_id})
                    event_hooks.fire('member_confirmed', {'community_id': community_id, 'building_id': building_id})
                    ...
                    if confirmed_count >= 3:
                        event_hooks.fire(
                            'formation_threshold_reached',
                            {'community_id': community_id, 'confirmed_count': confirmed_count},
                        )
        ...
        with db.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                db.store_leg_document(
                    community_id, 'community_agreement', agreement_pdf, f'vereinbarung_{community_id[:8]}.pdf'
                )
                ...
                for m in confirmed:
                    ...
                    db.store_leg_document(
                        community_id, 'participant_contract', pdf, f'vertrag_{m["building_id"][:8]}.pdf'
                    )
                ...
                db.store_leg_document(
                    community_id, 'dso_notification', dso_pdf, f'vnb_anmeldung_{community_id[:8]}.pdf'
                )
                ...
                if deepsign_key:
                    try:
                        ...
                        signers = [
                            {'name': m.get('address', m['building_id']), 'email': m['email']}
                            for m in confirmed
                            if m.get('email')
                        ]

                        ds_id = deepsign_integration.upload_document(
                            agreement_pdf, f'vereinbarung_{community_id[:8]}.pdf', f'Gemeinschaftsvereinbarung {name}'
                        )
...
def send_dso_email(to_email: str, subject: str, body: str, attachments: List[Dict]) -> bool:
    ...
    """
    try:
        from email_utils import EMAIL_ENABLED, FROM_EMAIL, SMTP_ENABLED, SMTP_HOST, SMTP_PASSWORD, SMTP_PORT, SMTP_USER
        ...
        with db.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if community.get('admin_building_id'):
                    cur.execute(
                        'SELECT city_id FROM buildings WHERE building_id = %s', (community['admin_building_id'],)
                    )
                    brow = cur.fetchone()
                    if brow:
...
def calculate_municipality_business_case(
    ...
    """
    per_household = calculate_savings_estimate(avg_consumption_kwh, avg_pv_kwp, avg_community_size)
...
def calculate_savings_estimate(
    consumption_kwh: float, pv_kwp: float, community_size: int, solar_kwh_per_kwp: int = 900
) -> Dict:
    """
