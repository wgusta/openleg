...
class TestCronFailClosed:
    ...
    def test_cron_refresh_fail_closed(self):
        with open(os.path.join(PROJECT_ROOT, "app.py")) as f:
            content = f.read()
        assert "_require_cron_secret" in content, \
            "Cron endpoints should use _require_cron_secret helper"
    ...
    def test_admin_no_query_param_token(self):
        ...
        for line in lines:
            if 'def _require_admin' in line:
                ...
                has_args_get = True
        assert not has_args_get, \
            "_require_admin still accepts token from query string"
    ...
    def test_utility_admin_no_query_param_token(self):
        ...
        for line in lines:
            if 'def admin_clients' in line:
                ...
                has_args_token = True
        assert not has_args_token, \
            "utility admin_clients still accepts token from query string"
...
class TestCommunityIdType:
    ...
    def test_billing_periods_community_id_varchar(self):
        ...
        assert "community_id INTEGER NOT NULL" not in content.split("billing_periods")[1].split("CREATE TABLE")[0] if "billing_periods" in content else True, \
    ...
    def test_invoices_community_id_varchar(self):
        ...
        for line in lines:
            if 'invoices' in line and 'CREATE TABLE' in line:
                ...
                uses_integer = True
        assert not uses_integer, \
            "invoices table still uses INTEGER community_id"
