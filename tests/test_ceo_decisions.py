...
class TestRequestApproval:
    def test_rejects_without_token(self, ceo_client):
        resp = ceo_client.post("/api/internal/request-approval",
...
def test_empty_webhook_secret_rejects(ceo_client):
    ...
    try:
        resp = ceo_client.post("/webhook/telegram",
...
def test_request_approval_escapes_summary(ceo_client):
    with patch("app._send_telegram_message", return_value=42) as mock_tg, \
         patch("database.create_ceo_decision", return_value=True):
        resp = ceo_client.post("/api/internal/request-approval",
                               json={"request_id": "esc-test", "activity": "outreach",
                                     "summary": "Email_to *user*", "reference": "test_ref",
                                     "payload": {"to": "x@y.z"}},
                               headers={"X-Internal-Token": "secret-internal"})
        assert resp.status_code == 200
...
def test_request_approval_conflict_returns_409(ceo_client):
    with patch("app._send_telegram_message", return_value=42), \
        ...
        resp = ceo_client.post("/api/internal/request-approval",
                               json={"request_id": "dup-test", "activity": "outreach",
                                     "summary": "Test", "payload": {"to": "x@y.z"}},
                               headers={"X-Internal-Token": "secret-internal"})
...
def test_failed_action_updates_status(ceo_client):
    fake_decision = {
        "request_id": "fail-123", "activity": "outreach",
...
def test_track_event_not_called_on_failure(ceo_client):
    import app as app_mod
    fake_decision = {
        "request_id": "track-test", "activity": "outreach",
...
def test_unknown_activity_reply(ceo_client):
    fake_decision = {
        "request_id": "unk-123", "activity": "data_export",
        "payload": {}, "status": "approved"
    }
    with patch("database.resolve_ceo_decision", return_value=fake_decision), \
...
def test_pending_command_lists_decisions(ceo_client):
    fake_decisions = [
        {"request_id": "test-1", "activity": "outreach", "summary": "Email", "created_at": "2026-01-01"}
    ]
    with patch("database.get_ceo_decisions", return_value=fake_decisions), \
         patch("app._send_telegram_message", return_value=None) as mock_tg:
        resp = ceo_client.post("/webhook/telegram",
