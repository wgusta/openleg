...
class TestOutreachQueueCRUD:
    ...
    def test_schedule_municipality_outreach_returns_id(self, mock_conn):
        ...
        mock_conn.return_value.__enter__ = lambda s: MagicMock(cursor=MagicMock(return_value=MagicMock(__enter__=lambda s: mock_cur, __exit__=MagicMock())))
...
class TestScoreOutreachCandidates:
    ...
    def test_score_outreach_candidates_basic(self):
        ...
        profiles = [
            {'name': 'A', 'contact_email': 'a@a.ch', 'energy_transition_score': 80, 'leg_value_gap_chf': 200, 'population': 50000},
            {'name': 'B', 'contact_email': 'b@b.ch', 'energy_transition_score': 40, 'leg_value_gap_chf': 100, 'population': 10000},
        ]
...
class TestScheduleOutreachBatch:
    ...
    def test_schedule_batch_picks_top_candidates(self, mock_profiles, mock_vnb, mock_history, mock_schedule):
        mock_profiles.return_value = [
            {'name': 'Dietikon', 'bfs_number': 261, 'kanton': 'ZH', 'energy_transition_score': 50,
             'leg_value_gap_chf': 200, 'population': 29000},
        ]
...
class TestProcessMunicipalityOutreach:
    ...
    def test_process_sends_pending(self, mock_pending, mock_mark, mock_send, mock_notify):
        mock_pending.return_value = [
            {'id': 1, 'municipality_name': 'Dietikon', 'bfs_number': 261, 'kanton': 'ZH',
             'contact_email': 'info@dietikon.ch', 'email_type': 'initial', 'followup_number': 0},
        ]
    ...
    def test_process_renders_template_with_app_context(self, full_app):
        ...
        with patch('email_automation.db.get_pending_municipality_outreach', return_value=pending_data), \
...
class TestFollowupScheduling14Day:
    ...
    def test_schedule_followups_14day(self, mock_needing, mock_schedule):
        mock_needing.return_value = [
            {'id': 2, 'municipality_name': 'Schlieren', 'bfs_number': 247, 'kanton': 'ZH',
...
class TestFollowupTemplates:
    ...
    def test_followup_1_renders(self, full_app):
        with full_app.app_context():
            ...
            html = render_template('emails/gemeinde_followup_1.html',
                                   gemeinde_name='Dietikon', kanton='ZH',
                                   energy_transition_score=42.5, leg_value_gap_chf=171,
    ...
    def test_followup_2_renders(self, full_app):
        with full_app.app_context():
            from flask import render_template
            html = render_template('emails/gemeinde_followup_2.html',
                                   gemeinde_name='Schlieren', kanton='ZH',
                                   energy_transition_score=38, leg_value_gap_chf=155,
