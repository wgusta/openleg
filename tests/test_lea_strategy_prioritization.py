...
def test_strategy_item_change_fires_event(strat_client):
    ...
    with patch('database.track_event'), patch('app._send_telegram_message'):
...
def test_strategy_done_notifies_telegram(strat_client):
    ...
    with patch('database.track_event'), patch('app._send_telegram_message') as mock_tg:
        strat_client.post(
            '/api/internal/notify-event',
            json={'event_type': 'strategy_status_changed', 'payload': {'item': 'test-item', 'status': 'done'}},
            headers={'X-Internal-Token': 'secret-internal'},
        )
...
def test_strategy_blocked_alerts_ceo(strat_client):
    ...
    with patch('database.track_event'), patch('app._send_telegram_message') as mock_tg:
        strat_client.post(
            '/api/internal/notify-event',
            json={'event_type': 'strategy_status_changed', 'payload': {'item': 'blocked-item', 'status': 'blocked'}},
            headers={'X-Internal-Token': 'secret-internal'},
        )
        mock_tg.assert_called_once()
