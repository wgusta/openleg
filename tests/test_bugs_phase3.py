...
class TestDuplicateVnbPipelineStats:
    ...
    def test_funnel_stats_renamed(self):
        with open(os.path.join(PROJECT_ROOT, 'database.py')) as f:
            content = f.read()
        assert 'def get_vnb_funnel_stats' in content, 'Second function should be renamed to get_vnb_funnel_stats'
...
class TestPublicApiRateLimit:
    ...
    def test_before_request_rate_limit(self):
        with open(os.path.join(PROJECT_ROOT, 'api_public.py')) as f:
            content = f.read()
        assert 'before_request' in content, 'Public API has no before_request rate limiting'
        assert '429' in content, 'Public API does not return 429 on rate limit'
...
class TestHttpTimeouts:
    ...
    def test_deepsign_timeouts(self):
        ...
        assert timeout_count >= call_count, f'Found {call_count} requests calls but only {timeout_count} timeouts'
